...
> NOTE: This is a hobby project; It does not guarantee a stable, bug-free experience. It is not recommended to host this on a public server and use it over the internet. I made this project for fun, and for my own little Minecraft SMP (Survival Multiplayer) project. For more serious projects, it is recommended to use [Pterodactyl](https://pterodactyl.io/). Again, I will not be responsible for any damage caused by this software, and it is up to you to use it responsibly.

## Features
- Info-rich dashboard, allowing a quick glance over server stats
- A nice view and editing interface for `server.properties`
- Easy world backups management
- Start and manage server, view players list, all from a single view
- Uses websockets if `flask-sock` is installed, otherwise falls back to API polling
...
## How to use
For now, no binaries are provided. A `build_installer.py` is provided for you to create your own binaries according to your ease.
But it's quite easy to run it yourself:

1. Install Python 3.11+ runtime (Tkinter is required, which should be installed on your system by default. If it isn't, install it using pip or if you're on Linux and pip method doesn't work, install it with your system's package manager).
...
1. Install the dependencies with `pip install flask flask-sock`.
...
4. The embedded Flask app streams status/logs over a WebSocket at `/ws` and forwards `POST /api/command` requests so the web UI and desktop UI stay in sync.

## Real-time player chat
If you run your server on Endstone, you can use the optional chat logger plugin in `endstone-chat-logger` to emit `[CHAT]` lines that the Web Manager chat panel ingests.

### Installing and using Endstone as the server backend
Endstone wraps the Bedrock Dedicated Server (BDS) and allows you to install plugins.
...
If Endstone ever says BDS is missing and asks to download it, most likely it's because your server folder selection/backend selection is wrong: the backend should be `endstone` and the server folder should be the BDS root (not a pre-made `bedrock_server/` directory).

### Installing real-time player chat plugin
To install the plugin, run the following command from repo root (use the same Python environment where Endstone is installed):

```bash
python3 -m pip install -e endstone-chat-logger
```

Restart your Endstone server after installing so the plugin is loaded and starts emitting `[CHAT] ...` lines.
